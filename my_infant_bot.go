...
var urlListFBO string = "https://api-seller.ozon.ru/v2/posting/fbo/list"
...
func main() {
	...
	router.HandleFunc("/sse", broker.ServeHTTP)
...
func (broker *Broker) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
	...
	for {
		...
		default:
			...
			if message := <-messageChan; message != nil {
				fmt.Fprintf(rw, "data: %s\n\n", message)
			} else {
				fmt.Fprintf(rw, ": nothing to sent\n\n")
			}
